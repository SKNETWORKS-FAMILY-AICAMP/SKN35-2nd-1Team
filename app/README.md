...
│  ├─ 1_dashboard.py       # 대시보드 — KPI 넷 + 도넛/막대 넷 (결과서 오면 피처 중요도)
│  ├─ 2_students.py        # 학생 목록 — 좁히기 → 줄을 눌러 상세 3탭
│  ├─ 3_risk_list.py       # 집중관리 대상 — 우선 처리 명단 · 상담 진행 상태
│  └─ 4_manual.py          # 예비학생 예측 — 명단에 없는 학생을 손으로 넣는다
├─ components/
...
│  ├─ manual_input.py      # 직접 입력 폼 (예비학생 예측 화면이 쓴다)
...
팀 초안(2026-08-28)에 맞춰 정리했습니다. 담당자의 하루가 곧 순서입니다.

| 화면 | 무엇을 하는가 |
|---|---|
| **메인** | 무슨 시스템인지 + 전체 학생 수 · 모델 F1 + 세 갈래 바로가기 |
| **대시보드** | 규모(KPI 넷) → 성격(도넛·막대 넷). 경고 띠 옆에 집중관리 바로가기 |
| **학생 목록** | 학번·전공·위험도로 좁히고, 줄을 누르면 상세 3탭 |
| **집중관리 대상** | 확률 내림차순 우선 처리 명단 + **상담 진행 상태** |
| **예비학생 예측** | 명단에 없는 학생의 값을 직접 넣어 위험도·지원 방향을 본다 |

> 직접 입력은 한때 학생 목록 안에 접어 두었는데 다시 제 화면으로 뺐습니다. 명단은
> **있는 학생을 찾는 곳**이고 예비학생 예측은 **없는 학생을 넣어 보는 곳**이라 하는
> 일이 다릅니다. 겹쳐 두면 매일 쓰는 명단 화면이 길어지기만 합니다.

...
### 데이터 관련 주의사항
학생 목록의 이름·학년은 원본(UCI) 데이터셋에 없는 값입니다. 원본이 개인정보 보호를 위해 완전히 익명화되어 있어 이름 컴럼 자체가 없고, "학년" 개념도 존재하지 않습니다. 화면 시연을 위해 학번(S0001~S0885, 행 순서 기반 부여)을 해시하여 이름을 생성했고 학년도 임의 배정했습니다. 실제 학생의 개인정보가 아닙니다.

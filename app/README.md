...
│  ├─ 2_prediction.py      # 예측 안 A — 한 화면 (입력 32개를 4탭으로)
│  ├─ 3_students.py        # 목록 — 툴바 필터 + master-detail (조치/근거/What-if 3탭)
│  ├─ 4_model.py           # 성능 — 팀 학습 결과서 + 정답 라벨 채점 + 임계값
│  └─ 5_prediction_steps.py # 예측 안 B — 단계형 (네 단계로 묻고 마지막에 전부)
├─ components/
│  ├─ theme.py             # ★ 디자인 시스템 (색·타이포·간격·상태) — 여기만 고친다
│  ├─ ui.py                # 공통 조각 (KPI · 위험/근거 미터 · 액션 카드 · 표 · 내려받기)
│  ├─ whatif.py            # What-if 시뮬레이션 패널
│  ├─ ab.py                # A/B 비교 안내 (결정되면 진 쪽과 함께 삭제)
...
**124개 통과** (로직 79 + 화면 45). 가장 중요한 것은 `TestPreprocessorContract` 입니다 —
...
## 1.6 지금 예측 화면이 두 개인 이유 (A/B · 임시)

같은 일을 하는 화면이 둘입니다. **입력을 받는 방식만** 다르고 예측기·규칙 엔진·결과
화면은 완전히 같습니다. 팀원이 둘 다 써 보고 고르기 위한 것이며, **정해지면 한쪽과
`components/ab.py` 를 함께 지웁니다.** 둘 다 남기면 화면이 늘어 복잡도가 도로 커집니다.

| 안 | 파일 | 방식 | 강점 | 약점 |
|---|---|---|---|---|
| **A** | `views/2_prediction.py` | 한 화면 · 4탭 | 값 고치고 바로 재분석. 클릭 1번 | 처음 보는 사람에겐 입력이 많아 보임 |
| **B** | `views/5_prediction_steps.py` | 네 단계 | 단계마다 **알아낸 것이 쌓여** 결과가 갑작스럽지 않음 | 결과까지 클릭 4번 |

> 🔴 **단계형에서 밟기 쉬운 함정** — Streamlit 은 이번 실행에서 그려지지 않은 위젯의
> `session_state` 를 버립니다. 1단계 값을 위젯 key 에만 두면 2단계로 넘어가는 순간
> **에러 없이** 기본값으로 되돌아갑니다. 그래서 값의 주인은 `wz_data` dict 이고,
> 매 실행 첫머리에 위젯 key 와 양방향으로 맞춥니다 (`init_state()`).
> 회귀 방지 테스트: `TestPredictionLayouts.test_next_advances_and_keeps_earlier_answers`

---

### 화면 구성 — 같은 내용을 두 순서로 쓴다

같은 결과를 **처음 보는 사람**과 **매일 쓰는 담당자**에게 같은 순서로 보여줄 이유가 없다.

| 화면 | 순서 | 왜 |
|---|---|---|
| 학생 위험 예측 | 얼마나 위험한가 → 왜 → 무엇을 (한 흐름) | 처음 설명할 때는 이 서사가 제품 자체다 |
| 학생 목록 상세 | **조치** / 근거 / What-if (탭) | 담당자에게 필요한 건 조치뿐이고, 근거는 요구받았을 때 편다 |

상세를 세로로 다 쌓으면 한 화면에 블록이 열 개를 넘어가고, 그러면 매일 쓰는 조치가
근거·시뮬레이션에 묻힌다. **내용을 버리지 않으면서 한 번에 보이는 양만 줄이는 방법**이
탭이라 셋으로 접었다(`ui.action_panel` · `ui.evidence_panel`).
